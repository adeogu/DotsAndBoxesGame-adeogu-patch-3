...
io.on("connection", (socket) => {
  ...
  socket.on("joinLobby", (data) => {
    ...
    if (lobbies[hostId]) {
      if (
        Object.keys(lobbies[hostId].members).length <
        lobbies[hostId].playerCount
      ) {
        ...
        if (
          ...
          for (let playingId of Object.keys(liveLobby.members)) {
            io.to(playingId).emit("startGame", {
              lobbies: lobbies,
              liveLobby: liveLobby,
            });
          }
  ...
  socket.on("makeMove", (data) => {
    ...
    if (
      lobbies[liveLobbyId] &&
      Object.keys(lobbies[liveLobbyId].members).length ==
        lobbies[liveLobbyId].playerCount
    ) {
      ...
      if (playerIndex + 1 == liveGameState.currentTurn) {
        ...
        let [validMove, currentMoves] = addMove(
          data.row,
          data.col,
          data.direction,
          liveLobbyId
        ...
        if (validMove) {
          ...
          if (completedCells.length != 0) {
            lobbies[liveLobbyId].gameState.points[moveTurn] +=
  ...
  socket.on("updateRematchForm", (data) => {
    lobbies[data.lobbyId].rematch[data.playerId] =
      !lobbies[data.lobbyId].rematch[data.playerId];
...
function deleteEmptyLobbies() {
  ...
  for (let lobbyId of lobbyIds) {
    ...
    let hostGone = !Object.keys(lobbies[lobbyId].members).includes(
...
function rematch(liveLobby) {
  if (!Object.values(liveLobby.rematch).includes(false)) {
    lobbies[liveLobby.hostId] = startOnlineGame(
      liveLobby,
      liveLobby.matchHistory
    );
    for (let playingId of Object.keys(liveLobby.members)) {
      io.to(playingId).emit("startGame", {
        lobbies: lobbies,
        liveLobby: lobbies[liveLobby.hostId],
      });
    }
  }
}
...
function buildMoves(matrix) {
  ...
  for (let ri = 0; ri <= rowCount; ri++) {
    for (let ci = 0; ci <= columnCount; ci++) {
      ...
      if (ci != columnCount) {
        if (ri == 0) {
          // up only, no duplicate
          allMoves[`${ri} ${ci} up`] = "";
        } else if (ri == rowCount) {
          // down only, no duplicate
      ...
      if (ri != rowCount) {
        if (ci == 0) {
          // left only, no duplicate
          allMoves[`${ri} ${ci} left`] = "";
        } else if (ci == columnCount) {
          // right only, no duplicate
